...
export async function resetTestData(sql: ReturnType<typeof postgres>) {
  await sql.unsafe(
    "TRUNCATE audit_log, rate_limit_buckets, password_reset_tokens, guest_appointment_management_tokens, guest_booking_confirmations, appointments, booking_slots, customers RESTART IDENTITY CASCADE",
  );
