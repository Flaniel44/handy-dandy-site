...
function AppointmentEditor({ appointment, save, showCustomer = true, reviewAction = null }: { appointment: Appointment; save: (id: string, notes: string, status: string, cancellationDiscountPercent?: number) => Promise<void>; showCustomer?: boolean; reviewAction?: ReviewTarget["action"] }) {
  ...
  return <article id={`appointment-${appointment.id}`} className={pending ? "is-awaiting-approval" : ""}><div className="appointment-summary"><div>{showCustomer && <strong>{appointment.customerName}</strong>}<span>{appointment.serviceName} · {formatDate(appointment.startsAt)}</span><span className="appointment-booked-date">Booked on {formatDate(appointment.createdAt)}</span>{showCustomer && <a href={`mailto:${appointment.customerEmail}`}>{appointment.customerEmail}</a>}</div><small>{pending ? "Awaiting approval" : appointment.source}</small></div>
    <p className="appointment-meeting-details"><strong>{appointment.appointmentMode === "in_person" ? "In person" : appointment.appointmentMode === "google_meet" ? "Google Meet" : "By phone"}</strong><span>{appointment.appointmentMode === "in_person" ? appointmentAddress || "Address not provided" : appointment.appointmentMode === "google_meet" ? "Join link is in the Google Calendar event" : appointment.appointmentPhone || appointment.customerPhone || "Phone not provided"}</span></p>
